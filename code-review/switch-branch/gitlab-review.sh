...
#
# Usage :
#   bash gitlab-review.sh <path> [base_branch]
#
...
# BASE_BRANCH est défini dynamiquement depuis les arguments (défaut: main)
...
main() {
  ...
  # ── 1. Arguments : chemin + branche de comparaison ─────
  local input_path BASE_BRANCH
  if [[ $# -gt 0 ]]; then
    input_path=$(realpath "$1")
  else
    echo -e "${RED}✗ Chemin requis en argument${NC}"
    echo "  Usage : bash gitlab-review.sh <path> [base_branch]"
    echo "  Exemples :"
    echo "    bash gitlab-review.sh /mon/projet/apps/remotes/payee-information"
    echo "    bash gitlab-review.sh /mon/projet/apps/remotes/payee-information develop"
    exit 1
  fi

  # Branche de comparaison (2ème argument, défaut : main)
  BASE_BRANCH="${2:-main}"
